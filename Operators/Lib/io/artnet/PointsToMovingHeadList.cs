...
using System;
using T3.Core;
using System.Collections.Generic; // Added for List<float>
...
public sealed class PointsToMovingHeadList : Instance<PointsToMovingHeadList>
{
    ...
    private void Update(EvaluationContext context)
    {
        ...
        try
        {
            ...
            bool needsBufferSetup = updateContinuously ||
            ...
            using (sourceStream)
            {
                ...
                List<float> resultItems = new List<float>(fixtureChannelSize * points.Length);
                ...
                for (var connectionIndex = 0; connectionIndex < points.Length; connectionIndex++)
                {
                    ...
                    List<float> items = new List<float>(fixtureChannelSize);
...
private void ProcessRotation(EvaluationContext context, Point point, List<float> items)
{
    ...
    if (!float.IsNaN(rotation.X) && !float.IsNaN(rotation.Y) && !float.IsNaN(rotation.Z) && !float.IsNaN(rotation.W))
    {
        ...
        Vector3 direction = Vector3.Transform(initialForwardAxis, rotation);
        ...
        float panMin = panRange.X * MathF.PI / 180f;
        float panMax = panRange.Y * MathF.PI / 180f;
        float tiltMin = tiltRange.X * MathF.PI / 180f;
        float tiltMax = tiltRange.Y * MathF.PI / 180f;
        ...
        {
            float prevPan = _lastPanTilt.X;
            float prevTilt = _lastPanTilt.Y;

            // Unwrap panValue to be as close as possible to prevPan (multi-turn support)
            float panSpan = panMax - panMin;
            float unwrappedPan = panValue;
            if (panSpan > MathF.PI * 1.5f) // More than 270°, allow multi-turn
            {
                ...
                float turns = MathF.Round((prevPan - panValue) / (2 * MathF.PI));
            ...
            float directPanDiff = MathF.Abs(unwrappedPan - prevPan);

            // Try the flip solution
            float flippedTilt = MathF.PI - tiltValue;
            float flippedPan = unwrappedPan + MathF.PI;
            // Wrap flippedPan into pan range
            while (flippedPan < panMin) flippedPan += 2 * MathF.PI;
            while (flippedPan > panMax) flippedPan -= 2 * MathF.PI;

            float flipPanDiff = MathF.Abs(flippedPan - prevPan);

            // Choose the solution with the smaller pan movement, but only if both are within panMin/panMax
            bool directValid = (unwrappedPan >= panMin && unwrappedPan <= panMax);
            bool flipValid = (flippedPan >= panMin && flippedPan <= panMax);
        ...
        else
        {
            ...
            {
                float panSpan = panMax - panMin;
                float turns = MathF.Round((_lastPanTilt.X - panValue) / (2 * MathF.PI));
    ...
    private void SetDmxCoarseFine(List<float> items, float value, int coarseChannel, int fineChannel, float inMin, float inMax, float maxDegrees)
    {
        ...
        int dmx16 = MapToDmx16(value, inMin, inMax, maxDegrees);

        if (fineChannel > 0)
        {
            // 16-bit mode
            int coarse = (dmx16 >> 8) & 0xFF;
            int fine = dmx16 & 0xFF;
        ...
        {
            // 8-bit mode
            int coarse = (int)Math.Round((dmx16 / 65535.0f) * 255.0f);
            InsertOrSet(items, coarseChannel - 1, coarse);
        }
    }
    ...
    private int MapToDmx16(float value, float inMin, float inMax, float maxDegrees)
    {
        ...
        if (Math.Abs(range) < 0.0001f || float.IsNaN(range) || Math.Abs(maxDegrees) < 0.0001f || float.IsNaN(maxDegrees))
            return 0;
        float normalizedValue = (value - inMin) / range;
        float degreeValue = normalizedValue * maxDegrees;
        float safeMaxDegrees = Math.Abs(maxDegrees) < 0.0001f ? 0.0001f : maxDegrees;
    ...
    private void ProcessColor(EvaluationContext context, Point point, List<float> items)
    {
        ...
        {
            float r = point.Color.X;
            float g = point.Color.Y;
            float b = point.Color.Z;
